...
tracking hypothesis support/contradiction across the Active Inference
literature.
"""
...
def plot_hypothesis_dashboard(
    ...
    ax.barh(y_positions, values, color=colors, edgecolor="white", linewidth=0.5,
            height=0.6)
...
def plot_evidence_timeline(
    ...
    if not yearly_scores:
        ax.text(0.5, 0.5, "No temporal evidence data", ha="center", va="center",
                fontsize=VIZ_CONFIG["font_size"])
        ax.set_axis_off()
        fig.savefig(output_path, dpi=VIZ_CONFIG["dpi"], bbox_inches="tight")
        plt.close(fig)
...
def plot_assertion_type_breakdown(
    ...
    if not assertion_counts:
        ax.text(0.5, 0.5, "No assertion data", ha="center", va="center",
                fontsize=VIZ_CONFIG["font_size"])
        ax.set_axis_off()
        fig.savefig(output_path, dpi=VIZ_CONFIG["dpi"], bbox_inches="tight")
        plt.close(fig)
    ...
    ax.barh(y_pos, supports, height=bar_height, color="#009E73",
            label="Supports", edgecolor="white", linewidth=0.5)
    ax.barh(y_pos, contradicts, height=bar_height, left=supports,
            color="#D55E00", label="Contradicts", edgecolor="white", linewidth=0.5)
    left_offset = [s + c for s, c in zip(supports, contradicts)]
    ax.barh(y_pos, neutrals, height=bar_height, left=left_offset,
            color="#56B4E9", label="Neutral", edgecolor="white", linewidth=0.5)
...
def plot_assertion_summary(
    ...
    if type_counts and total_assertions > 0:
        ...
        ax1.text(0.5, 0.5, "No assertions", ha="center", va="center",
                 fontsize=VIZ_CONFIG["font_size"])
        ax1.set_axis_off()
    ...
    if hypothesis_counts:
        ...
        colors_bar = [VIZ_CONFIG["palette"][i % len(VIZ_CONFIG["palette"])]
                      for i in range(len(hyps))]

        y_pos = range(len(hyps))
        bars = ax2.barh(y_pos, counts, color=colors_bar,
        ...
        ax2.text(0.5, 0.5, "No hypothesis data", ha="center", va="center",
                 fontsize=VIZ_CONFIG["font_size"])
        ax2.set_axis_off()
