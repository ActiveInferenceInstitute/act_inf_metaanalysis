"""Temporal trend visualizations for Active Inference literature.
...
def plot_growth_curve(
    ...
    if smoothed_annual:
        ...
        ax1.plot(years, smooth_vals, color=trend_color, linewidth=2.5, linestyle="--",
                 label="Trend (Moving Avg)")

    ax1.set_xlabel("Year", fontsize=VIZ_CONFIG["font_size"])
    ax1.set_ylabel("Annual Publications", color=bar_color,
    ...
    ax2.plot(years, cumul, color=line_color, linewidth=2.5, marker="o",
             markersize=5, label="Cumulative")
    ax2.set_ylabel("Cumulative Publications", color=line_color,
                    fontsize=VIZ_CONFIG["font_size"])
    ax2.tick_params(axis="y", labelcolor=line_color)

    ax1.set_title(
        "Active Inference Publication Growth",
