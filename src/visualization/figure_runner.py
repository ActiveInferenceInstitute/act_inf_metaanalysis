...
def generate_all_figures(args: argparse.Namespace) -> list[str]:
    ...
    if subfield_data:
        ...
        generated_paths.append(
            str(
                plot_field_summary(
                    total_papers, subfield_data, output_dir / "field_summary.png"
                )
            )
        )
        generated_paths.append(
            str(
                plot_subfield_distribution(
                    subfield_data, output_dir / "subfield_distribution.png"
                )
            )
        )
    ...
    if temporal_data and "year_counts" in temporal_data:
        ...
        smoothed = {
            int(k): v for k, v in temporal_data.get("smoothed_annual", {}).items()
        }
        generated_paths.append(
            str(
                plot_growth_curve(
                ...
                generated_paths.append(
                    str(
                        plot_degree_distribution(
                            graph, output_dir / "degree_distribution.png"
                        )
                    )
                )
    ...
    if scores_data:
        generated_paths.append(
            str(
                plot_hypothesis_dashboard(
                    scores_data, output_dir / "hypothesis_dashboard.png"
                )
            )
        )
    ...
    if isinstance(topics_data, list) and topics_data:
        ...
        if word_weights:
            generated_paths.append(
                str(plot_word_cloud(word_weights, output_dir / "word_cloud.png"))
            )
        generated_paths.append(
            str(plot_topic_term_bars(topics_data, output_dir / "topic_term_bars.png"))
        )
    ...
    if tfidf_data and "matrix" in tfidf_data:
        ...
        if tfidf_matrix.shape[0] >= 2 and doc_labels:
            generated_paths.extend(
                [
                    ...
                    str(
                        plot_dendrogram(
                            tfidf_matrix, doc_labels, output_dir / "dendrogram.png"
                        )
                    ),
                ]
            )
        if doc_tokens:
            generated_paths.append(
                str(
                    plot_cooccurrence_matrix(
                        doc_tokens, output_dir / "cooccurrence_matrix.png"
                    )
                )
            )
...
def _register_figures(
    generated_paths: list[str], output_dir: Path, logger: logging.Logger
) -> None:
    try:
