"""Query helper functions for the Active Inference knowledge graph.
...
from knowledge_graph.schema import ASSERTION_TYPES, HYPOTHESIS_CATEGORIES
...
def query_supporting_papers(kg: KnowledgeGraph, hypothesis_id: str) -> list[str]:
    ...
    for aid, assertion in kg._assertion_map.items():
        if (
            assertion.hypothesis_id == hypothesis_id
            and assertion.assertion_type == "supports"
        ):
...
def query_contradicting_papers(kg: KnowledgeGraph, hypothesis_id: str) -> list[str]:
    ...
    for aid, assertion in kg._assertion_map.items():
        if (
            assertion.hypothesis_id == hypothesis_id
            and assertion.assertion_type == "contradicts"
        ):
            paper_ids.add(assertion.paper_id)
    return sorted(paper_ids)
