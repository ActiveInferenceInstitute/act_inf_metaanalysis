...
triple patterns of the Active Inference meta-analysis:
...
class KnowledgeGraph:
    """Knowledge graph for Active Inference literature.
    ...
    def add_paper(self, paper: Paper) -> None:
        ...
        if self._use_rdflib:
            ...
            self._rdf_graph.add(
                (subj, URIRef(f"{AIF_NAMESPACE}type"), URIRef(f"{AIF_NAMESPACE}Paper"))
            )
            self._rdf_graph.add(
                (subj, URIRef(f"{AIF_NAMESPACE}title"), Literal(paper.title))
            )
        else:
            self._nx_graph.add_node(
                _paper_uri(pid), node_type="Paper", title=paper.title, paper_id=pid
            )
    ...
    def add_assertion(self, assertion: Assertion) -> None:
        ...
        if self._use_rdflib:
            ...
            self._rdf_graph.add(
                (URIRef(a_uri), URIRef(f"{AIF_NAMESPACE}claim"), Literal(assertion.claim))
            )
            if rel_pred and h_uri:
                self._rdf_graph.add(
                    (URIRef(a_uri), URIRef(rel_pred), URIRef(h_uri))
                )
        else:
            self._nx_graph.add_node(
                a_uri,
    ...
    def add_subfield(self, paper_id: str, subfield: str) -> None:
        ...
        if self._use_rdflib:
            self._rdf_graph.add(
                (URIRef(p_uri), URIRef(belongs_pred), URIRef(sf_uri))
            )
        else:
