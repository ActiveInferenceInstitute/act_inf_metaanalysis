...
def _write_full_analysis_inputs(input_dir: Path) -> None:
    ...
    with open(input_dir / "hypothesis_scores.json", "w", encoding="utf-8") as handle:
        json.dump({"FEP_UNIVERSALITY": 0.5, "SCALABILITY": 0.3}, handle)
    with open(input_dir / "hypothesis_trends.json", "w", encoding="utf-8") as handle:
        json.dump({"FEP_UNIVERSALITY": {"2019": 0.1, "2020": 0.4}}, handle)
        ...
        json.dump(
            {
                ...
                "per_hypothesis": {
                    "FEP_UNIVERSALITY": {"supports": 2, "contradicts": 1},
        ...
        json.dump(
            {
                "total_assertions": 0,
                "type_counts": {},
                "per_hypothesis": {"FEP_UNIVERSALITY": {"supports": 0}},
            },
            handle,
        )
