...
class TestSearchArxiv:
    ...
    def test_search_returns_papers(self, httpserver: HTTPServer):
        ...
        httpserver.expect_request("/api/query").respond_with_data(
            SINGLE_ENTRY_XML, content_type="application/atom+xml"
        )
    ...
    def test_search_empty_results(self, httpserver: HTTPServer):
        ...
        httpserver.expect_request("/api/query").respond_with_data(
            EMPTY_FEED_XML, content_type="application/atom+xml"
        )
    ...
    def test_search_multiple_results(self, httpserver: HTTPServer):
        ...
        httpserver.expect_request("/api/query").respond_with_data(
            MULTI_ENTRY_XML, content_type="application/atom+xml"
        )
...
class TestArxivPaginationAndRetry:
    ...
    def test_pagination(self, httpserver: HTTPServer):
        ...
        page1_xml = header + "".join([
            entry_tmpl.format(id=f"1001.{i:04d}", title=f"Paper {i}")
            for i in range(100)
        ]) + footer
        
        # Page 2: 50 results (partial page)
        page2_xml = header + "".join([
            entry_tmpl.format(id=f"2002.{i:04d}", title=f"Paper {i+100}")
            for i in range(50)
        ]) + footer
