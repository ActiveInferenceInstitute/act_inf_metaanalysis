...
Uses real computation (no mocks) with the conftest fixtures.
"""
...
class TestLiteratureSearchScript:
    ...
    def test_main_with_all_sources_skipped(self, tmp_output_dir):
        ...
        with patch(
            ...
            ["01_literature_search.py",
             "--skip-arxiv", "--skip-s2", "--skip-openalex",
    ...
    def test_parse_args_defaults(self):
        ...
        with patch("sys.argv", ["01_literature_search.py"]):
            args = mod.parse_args()
    ...
    def test_resume_loads_existing_corpus(self, sample_papers, tmp_output_dir):
        ...
        # Pre-create a corpus with known papers
        corpus = Corpus()
        ...
        # Corpus should still have the same papers (no new sources)
    ...
    def test_log_level_flag_accepted(self):
        ...
        with patch("sys.argv", ["01_literature_search.py", "--log-level", "DEBUG"]):
            args = mod.parse_args()
...
class TestMetaAnalysisPipelineScript:
    ...
    def test_main_with_sample_corpus(self, sample_papers, tmp_output_dir):
        ...
        # Create a corpus file from sample papers
        corpus = Corpus()
        for paper in sample_papers:
            corpus.add(paper)
        ...
        spec = __import__("importlib").util.spec_from_file_location(
            "meta_analysis_pipeline", SCRIPTS_DIR / "02_meta_analysis_pipeline.py"
        )
        mod = __import__("importlib").util.module_from_spec(spec)
...
class TestBuildKnowledgeGraphScript:
    ...
    def test_main_generates_llm_assertions(self, sample_papers, tmp_output_dir,
                                           httpserver):
        ...
        # Serve a real Ollama-format JSON response from pytest-httpserver
        ollama_response = json.dumps({
            "response": json.dumps([
                {"hypothesis_id": "FEP_UNIVERSALITY", "direction": "supports",
                 "confidence": 0.8, "reasoning": "test evidence"},
            ]),
        ...
        ).respond_with_data(ollama_response, content_type="application/json")

        spec = __import__("importlib").util.spec_from_file_location(
            "build_knowledge_graph", SCRIPTS_DIR / "03_build_knowledge_graph.py"
        )
        mod = __import__("importlib").util.module_from_spec(spec)

        # Point --llm-url at the local httpserver (real HTTP, no mock)
        with patch(
            "sys.argv",
            ["03_build_knowledge_graph.py",
             "--corpus", str(corpus_path),
             "--output-dir", tmp_output_dir,
             "--llm-url", httpserver.url_for(""),
             "--clear-assertions"],
        ):
            spec.loader.exec_module(mod)
            mod.main()
        ...
        scores = json.loads(
            (data_dir / "hypothesis_scores.json").read_text()
        )
...
class TestGenerateFiguresScript:
    ...
    def test_main_with_minimal_data(self, tmp_output_dir):
        ...
        # Create minimal subfield classification
        ...
        # Create minimal hypothesis scores
        with open(input_dir / "hypothesis_scores.json", "w") as f:
            json.dump({
                "FEP_UNIVERSALITY": 0.65,
                "SCALABILITY": 0.3,
                "MARKOV_BLANKET_REALISM": -0.2,
            }, f)

        spec = __import__("importlib").util.spec_from_file_location(
            "generate_figures", SCRIPTS_DIR / "04_generate_figures.py"
        )
        mod = __import__("importlib").util.module_from_spec(spec)
        ...
        # At minimum, field_summary and hypothesis_dashboard should exist
