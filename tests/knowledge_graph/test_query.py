...
def _make_assertion(
    ...
    hypothesis_id: str = "FEP_UNIVERSALITY",
) -> Assertion:
...
def populated_kg(use_rdflib: bool) -> KnowledgeGraph:
    ...
    Structure:
        Papers: p1, p2, p3, p4
        Assertions:
            qa1: p1 supports FEP_UNIVERSALITY
            qa2: p2 supports FEP_UNIVERSALITY
            qa3: p3 contradicts FEP_UNIVERSALITY
    ...
    kg.add_assertion(_make_assertion("qa1", "qp1", "supports", "FEP_UNIVERSALITY"))
    kg.add_assertion(_make_assertion("qa2", "qp2", "supports", "FEP_UNIVERSALITY"))
    kg.add_assertion(_make_assertion("qa3", "qp3", "contradicts", "FEP_UNIVERSALITY"))
...
class TestQueryPapersByHypothesis:
    ...
    def test_fep_returns_three_papers(self, populated_kg: KnowledgeGraph) -> None:
        """FEP_UNIVERSALITY has assertions from p1, p2, p3."""
        result = query_papers_by_hypothesis(populated_kg, "FEP_UNIVERSALITY")
        assert len(result) == 3
    ...
    def test_unused_hypothesis_returns_empty(self, populated_kg: KnowledgeGraph) -> None:
        ...
        result = query_papers_by_hypothesis(populated_kg, "LANGUAGE_AIF")
        assert result == []
...
class TestQuerySupportingPapers:
    ...
    def test_fep_supporting(self, populated_kg: KnowledgeGraph) -> None:
        """p1 and p2 support FEP_UNIVERSALITY."""
        result = query_supporting_papers(populated_kg, "FEP_UNIVERSALITY")
        assert len(result) == 2
        assert "doi:10.5555/qp1" in result
        assert "doi:10.5555/qp2" in result
...
class TestQueryContradictingPapers:
    ...
    def test_fep_contradicting(self, populated_kg: KnowledgeGraph) -> None:
        """p3 contradicts FEP_UNIVERSALITY."""
        result = query_contradicting_papers(populated_kg, "FEP_UNIVERSALITY")
