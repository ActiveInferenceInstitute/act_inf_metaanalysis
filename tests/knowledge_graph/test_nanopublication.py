...
class TestAssertionDataclass:
    ...
    def test_all_fields_set(self) -> None:
        ...
        assert a.hypothesis_id == "FEP_UNIVERSALITY"
...
class TestJSONLSerialization:
    ...
    def test_round_trip_multiple(self, tmp_path: Path) -> None:
        ...
        assertions = [
            _make_assertion(assertion_id=f"ser{i}", citation_count=i * 10)
            for i in range(5)
        ]
...
class TestMergeNanopubs:
    ...
    def test_duplicate_key_new_wins(self) -> None:
        ...
        old = _make_assertion(
            paper_id="doi:10.1/a", hypothesis_id="H1", claim="old claim"
        )
        new = _make_assertion(
            paper_id="doi:10.1/a", hypothesis_id="H1", claim="new claim"
        )
...
class TestAppendNanopubs:
    ...
    def test_deduplicates_new_wins(self, tmp_path: Path) -> None:
        ...
        old = _make_assertion(
            paper_id="doi:10.1/a", hypothesis_id="H1", claim="old"
        )
        serialize_nanopubs([create_nanopub(old)], p)

        new = _make_assertion(
            paper_id="doi:10.1/a", hypothesis_id="H1", claim="new"
        )
...
class TestNanopubRDF:
    ...
    def test_trig_roundtrip_two_nanopubs(self, tmp_path: Path) -> None:
        ...
        a1 = _make_assertion(paper_id="p1", hypothesis_id="FEP_UNIVERSALITY")
        ...
        content = trig_path.read_text(encoding="utf-8")
        assert "FEP_UNIVERSALITY" in content or "fep_universality" in content
