...
class TestGetHypothesisById:
    ...
    def test_existing(self) -> None:
        ...
        h = get_hypothesis_by_id("FEP_UNIVERSALITY")
        assert h is not None
        assert h.hypothesis_id == "FEP_UNIVERSALITY"
        assert "FEP" in h.name
...
class TestScoreHypothesis:
    ...
    Setup:
        3 supporting assertions for FEP_UNIVERSALITY:
            confidence=1.0, citations=[10, 50, 100]
        1 contradicting assertion for FEP_UNIVERSALITY:
    ...
    def test_no_assertions_returns_zero(self) -> None:
        ...
        assertions = self._build_assertions()
        score = score_hypothesis(assertions, "LANGUAGE_AIF")
        assert score == 0.0

    def test_empty_list_returns_zero(self) -> None:
        """An empty assertion list should yield 0.0."""
        assert score_hypothesis([], "FEP_UNIVERSALITY") == 0.0
...
class TestTemporalTrend:
    ...
    def test_final_year_includes_all(self) -> None:
        ...
        papers, assertions = self._build_papers_and_assertions()
        trend = temporal_trend(assertions, "FEP_UNIVERSALITY", papers)
        global_score = score_hypothesis(assertions, "FEP_UNIVERSALITY")
